...
export class ProductDetailPage {
  ...
  public loaderWcVendorInfo = true;
  public wcVendorInfo = {};
  ...
  quantityChange() {
    ...
    if (this.product.stock_quantity == null) { console.log("quantity is unlimited"); }
      ...
      this.translate.get("Product Quantity is Limited!").subscribe((res) => {
    ...
    if (this.quantity == null || this.quantity == 0) { this.quantity = 1; }
    this.applicationRef.tick();
  }
  ...
  share() {
    ...
    this.socialSharing.shareViaWhatsApp('Check out the Ionic Academy!', null, 'https://ionicacademy.com'
    ).then(() => { // Success 
    }).catch((e) => { 
      // Error! 
    }); 
  ...
  openStore(value) {
    if (this.config.showWcVendorInfo) {
      this.navCtrl.push(StorePage, { data: value });
    }
    else {
      this.loading.show();
  ...
  getWcVendorInfo() {
    ...
    this.http.get(this.config.url + "/api/appsettings/get_vendor_info/?insecure=cool&product_id=" + this.product.id).map(res => res.json()).subscribe(data => {
      this.loaderWcVendorInfo = false;
      let d = data;
      this.wcVendorInfo = data.data;
      this.applicationRef.tick();
    });
  }
  ...
  share_product(detail) {
    this.loading.autoHide(2000);
    this.socialSharing.shareViaWhatsApp('', detail.images[0].src, "" 
    ).then(() => {  
      // Success 
    }).catch((e) => { 
      // Error! 
